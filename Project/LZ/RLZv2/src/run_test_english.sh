...
echo "threads,time,ref_mb,n_partitions,n_blocks,cache-misses,size" >> ${outfile}

iterations=3
dataset="../../../../Datasets/english.rep"
for REF_MB in 50 # Reference size
do
    for N_PART in 10 20 30 # number of partitions
    do
	for N_BLOCKS in 2 6 12 # number of blocks
	do
	    for (( i=0; i<${iterations}; i++)) # repetitions
	    do
	        for j in 1 2 4 6 8 10 12 # iterate over cores
	        do
		    let MAX_MEM_MB=$((5 * ${REF_MB} + 2100 / ${N_BLOCKS} +1)) # Calculate memory for N_BLOCKS
		    echo -n ${j}, >> ${outfile}
